...
void PrintObject::make_perimeters()
{
    ...
    // ORCA: Apply sacrificial bridges modification
    this->apply_sacrificial_bridges();
...
// ORCA: Tangential sacrificial bridging
void PrintObject::apply_sacrificial_bridges()
{
    ...
    for (size_t i = 2; i < m_layers.size(); ++i) {
        ...
        for (const ExPolygon& ov : overhangs) {
            ...
            // offset_ex on an ExPolygon expands both contour and holes (shrinking holes).
            // We want to expand the CONTOUR but keep the HOLE fixed (or shrink it slightly to overlap? No, keep fixed for tangential).
            // Actually, we want to fill the void (Overhang) AND anchor.
            // So we take the Overhang polygon, and Union it with an Offset of itself?
            // Or just Offset the Contour and subtract the original Hole?
